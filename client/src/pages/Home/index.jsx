...
export default function Home() {
  ...
  if (isLoading) return <Loader fullScreen width={20} height={20} />;

  return (
    <>
      <main className='relative flex flex-col justify-center gap-4 px-8 md:px-12 py-14 homeBg'>
        <div className='flex flex-col items-center gap-6'>
          <img className='w-28 h-28' src={codeBuddy} alt='logo' />
          <h1 className='max-w-3xl text-4xl font-black text-center text-gray-800 md:text-5xl'>
            Helps you to code, share the code and watch out contests
          </h1>
          <p className='max-w-2xl text-sm text-center opacity-75 md:text-md'>
...
const ScreenshotCard = ({ img, content }) => {
  ...
  return (<>
    <div className='relative max-w-4xl mx-auto my-14'>
      <div className='z-10 absolute px-6 py-1 md:py-4 bg-white border rounded-r-full rounded-l-xl drop-shadow-2xl bottom-6 -left-20'>
        <code>
          <span className='mr-2 opacity-60'>$</span>
          <i className='opacity-80'> {content}</i>
        </code>
      </div>
      <header className='flex gap-2 px-8 py-2 md:py-5 rounded-t-3xl bg-dark'>
        <span
